         ...
         return TimelineView(.animation) { _ in
             ...
             ZStack {
                ...
                // LineShape(yValues: list, scaleFactor: height / max(2, maxValue)).stroke(Color(rgb: 0x464859), lineWidth: 1.0)
                 LineShape(yValues: list2,lastPoint: CGPoint(x: 0, y: list2.last ?? 0), scaleFactor: height / max(2, maxValue)).stroke(Asset.Colors.primaryColor.swiftUIColor, lineWidth: 1.0)
